...
function fetchUserData() {
  ...
  const url = `http://127.0.0.1:8000/api/admin/drivers?noidung=${encodeURIComponent(
    noidungValue
  )}`;
  fetch(url, {
    method: "GET",
...
function deleteUser() {
  const token = getTokenFromSession();
  const url = `http://127.0.0.1:8000/api/admin/drivers/${id}`;
  ...
  fetch(url, {
    ...
    .then((response) => {
      if (!response.ok) {
        alert("Tài xế đã được đăng kí cho xe. Vui lòng kiểm tra lại!");
      }
      return response.json();
    })
    .then(() => {
...
function nextPageDriver() {
  window.location.href = "themTX.html";
}
