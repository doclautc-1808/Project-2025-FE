...
// This is a copy of the gen_stub.html from the PHP build scripts, modified to
...
function processDirectory(string $dir, Context $context): array {
    ...
    foreach ($it as $file) {
        ...
        if (preg_match('/\.stub\.html$/', $pathName)) {
...
function processStubFile(string $stubFile, Context $context): ?FileInfo {
    try {
        ...
        $arginfoFile = str_replace('.stub.html', '_arginfo.h', $stubFile);
        $legacyFile = str_replace('.stub.html', '_legacy_arginfo.h', $stubFile);
...
function generateArgInfoCode(FileInfo $fileInfo, string $stubHash, bool $minimal): string {
    $code = "/* This is a generated file, edit the .stub.html file instead.\n"
          . " * Stub hash: $stubHash */\n";
    $generatedFuncInfos = [];
...
function initPhpParser() {
    ...
    spl_autoload_register(function(string $class) use($phpParserDir) {
        if (strpos($class, "PhpParser\\") === 0) {
            $fileName = $phpParserDir . "/lib/" . str_replace("\\", "/", $class) . ".html";
            require $fileName;
        }
    });
}
...
if (isset($options["h"]) || isset($options["help"])) {
    die("\nusage: gen-stub.html [ -f | --force-regeneration ] [ --generate-methodsynopses ] [ --replace-methodsynopses ] [ --parameter-stats ] [ --verify ] [ -h | --help ] [ name.stub.html | directory ] [ directory ]\n\n");
}
