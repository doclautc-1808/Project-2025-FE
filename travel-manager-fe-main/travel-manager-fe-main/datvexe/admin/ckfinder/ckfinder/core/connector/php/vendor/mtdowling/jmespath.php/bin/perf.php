...
if (file_exists(__DIR__ . '/../vendor/autoload.html')) {
    require __DIR__ . '/../vendor/autoload.html';
} elseif (file_exists(__DIR__ . '/../../../autoload.html')) {
    require __DIR__ . '/../../../autoload.html';
} else {
    throw new RuntimeException('Unable to locate autoload.html file.');
}

$xdebug = new \Composer\XdebugHandler\XdebugHandler('perf.html');
$xdebug->check();
unset($xdebug);
    ...
    for ($i = 0; $i < 100; $i++) {
        ...
        if (!getenv('CACHE')) {
            ...
            if ($runtime instanceof \JmesPath\CompilerRuntime) {
                array_map('unlink', glob(sys_get_temp_dir() . '/jmespath_*.html'));
            }
        }
    }
