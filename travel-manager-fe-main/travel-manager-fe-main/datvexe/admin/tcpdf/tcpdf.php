...
// TCPDF configuration
require_once(dirname(__FILE__).'/tcpdf_autoconfig.html');
// TCPDF static font methods and data
require_once(dirname(__FILE__).'/include/tcpdf_font_data.html');
// TCPDF static font methods and data
require_once(dirname(__FILE__).'/include/tcpdf_fonts.html');
// TCPDF static color methods and data
require_once(dirname(__FILE__).'/include/tcpdf_colors.html');
// TCPDF static image methods and data
require_once(dirname(__FILE__).'/include/tcpdf_images.html');
// TCPDF static methods and data
require_once(dirname(__FILE__).'/include/tcpdf_static.html');
...
class TCPDF {
		...
		if (($fontsdir = opendir(TCPDF_FONTS::_getfontpath())) !== false) {
			while (($file = readdir($fontsdir)) !== false) {
				if (substr($file, -4) == '.html') {
					array_push($this->fontlist, strtolower(basename($file, '.html')));
				}
			}
			closedir($fontsdir);
		}
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (TCPDF_STATIC::empty_string($fontfile) OR (!@TCPDF_STATIC::file_exists($fontfile))) {
			...
			$tmp_fontfile = str_replace(' ', '', $family).strtolower($style).'.html';
			$fontfile = TCPDF_FONTS::getFontFullPath($tmp_fontfile, $fontdir);
			if (TCPDF_STATIC::empty_string($fontfile)) {
				$missing_style = true;
				// try to remove the style part
				$tmp_fontfile = str_replace(' ', '', $family).'.html';
				$fontfile = TCPDF_FONTS::getFontFullPath($tmp_fontfile, $fontdir);
			}
		}
		// include font file
	...
	public function setProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		if ($mode > 1) { // AES
			if (!extension_loaded('openssl') && !extension_loaded('mcrypt')) {
				$this->Error('AES encryption requires openssl or mcrypt extension (http://www.html.net/manual/en/mcrypt.requirements.html).');
			}
			...
			if (($mode == 3) AND !function_exists('hash')) {
				...
				$this->Error('AES 256 encryption requires HASH Message Digest Framework (http://www.html.net/manual/en/book.hash.html).');
			}
		}
		if ($owner_pass === null) {
	 ...
	 * @param string $tsa_username Specifies the username for TSA authorization (optional) OR specifies the TSA authorization PEM file (see: example_66.html, optional)
	 ...
	 * @param string $type type of barcode (see tcpdf_barcodes_1d.html for supported formats).
	...
	public function write1DBarcode($code, $type, $x=null, $y=null, $w=null, $h=null, $xres=null, $style=array(), $align='') {
		...
		require_once(dirname(__FILE__).'/tcpdf_barcodes_1d.html');
		// save current graphic settings
		$gvars = $this->getGraphicVars();
	 ...
	 * Print 2D Barcode.
	 * @param string $code code to print
	 * @param string $type type of barcode (see tcpdf_barcodes_2d.html for supported formats).
	 * @param float|null $x x position in user units
	 * @param float|null $y y position in user units
	...
	public function write2DBarcode($code, $type, $x=null, $y=null, $w=null, $h=null, $style=array(), $align='', $distort=false) {
		...
		require_once(dirname(__FILE__).'/tcpdf_barcodes_2d.html');
		// save current graphic settings
		$gvars = $this->getGraphicVars();
		// create new barcode object
