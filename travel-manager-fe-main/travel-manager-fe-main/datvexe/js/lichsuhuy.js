document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function loadTickets() {
    try {
      ...
      const scheduleResponse = await fetch(
        `http://127.0.0.1:8000/api/schedule/reserved-schedule?user_id=${userId}&is_canceled=true`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      `
        ...
        ticketRow.innerHTML = `
          ...
          scheduleDetails.arrival_location
            ...
            ${carDetails}
            <div style="margin-top:1rem;margin-left:0.5rem; color:red border:2px solid black;" id="countdown"></div>
  ...
  async function cancelTicket(reservationId) {
    const url = `http://127.0.0.1:8000/api/schedule/reservation-status?reservation_id=${reservationId}&is_accepted=true`;
