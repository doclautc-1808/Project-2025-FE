document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function loadTickets() {
    try {
      ...
      const scheduleResponse = await fetch(
        `http://127.0.0.1:8000/api/schedule/reserved-schedule?user_id=${userId}&is_canceled=false`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        ticketRow.innerHTML = `
          ...
          scheduleDetails.arrival_location
            ...
            <ul class="list-unstyled booking-info">
              ...
              <li><span class="line">Phương thức thanh toán: </span>${
                schedule.method
              }</li>
            </ul>
            <hr style="height:5px;border-width:0;color:gray;background-color:white">
            ...
            <a href="suave.html?id=${
              schedule.reservation_id
            }" class="btn btn-orange">Sửa thông tin</a>
            
          </td>
          <td class="dash-list-btn" style="border-bottom: 2px solid black">
            <a class="btn btn-orange cancel-btn" data-reservation-id="${
              schedule.reservation_id
            }">Huỷ vé</a>
          </td>
        `;
  ...
  async function cancelTicket(reservationId) {
    const url = `http://127.0.0.1:8000/api/schedule/reservation-status?reservation_id=${reservationId}&is_accepted=false`;
