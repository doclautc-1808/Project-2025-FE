...
function MakeFont($fontfile, $enc='cp1252', $embed=true, $subset=true)
{
	...
	MakeDefinitionFile($basename.'.html', $type, $enc, $embed, $subset, $map, $info);
	Message('Font definition file generated: '.$basename.'.html');
}
...
if(PHP_SAPI=='cli')
{
	...
	if($argc==1)
		die("Usage: php makefont.html fontfile [encoding] [embed] [subset]\n");
	$fontfile = $argv[1];
	if($argc>=3)
		$enc = $argv[2];
	else
