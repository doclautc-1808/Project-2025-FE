...
function AddFont($family, $style='', $file='', $uni=false)
{
	...
	if($file=='') {
	   if ($uni) {
		...
		$file = str_replace(' ','',$family).strtolower($style).'.html';
	   }
	}
	...
	if ($uni) {
		...
		if (file_exists($unifilename.'.mtx.html')) {
			include($unifilename.'.mtx.html');
		}
		if (!isset($type) ||  !isset($name) || $originalsize != $ttfstat['size']) {
			$ttffile = $ttffilename;
			require_once($this->fontpath.'unifont/ttfonts.html');
			$ttf = new TTFontFile();
			...
			if (is_writable(dirname($this->fontpath.'unifont/'.'x'))) {
				$fh = fopen($unifilename.'.mtx.html',"w");
				fwrite($fh,$s,strlen($s));
				fclose($fh);
				$fh = fopen($unifilename.'.cw.dat',"wb");
				fwrite($fh,$cw,strlen($cw));
				fclose($fh);
				@unlink($unifilename.'.cw127.html');
			}
			unset($ttf);
		}
		else {
...
protected function _putTTfontwidths(&$font, $maxUni) {
	if (file_exists($font['unifilename'].'.cw127.html')) {
		include($font['unifilename'].'.cw127.html') ;
		$startcid = 128;
	}
	else {
		$rangeid = 0;
	...
	for ($cid=$startcid; $cid<$cwlen; $cid++) {
		if ($cid==128 && (!file_exists($font['unifilename'].'.cw127.html'))) {
			if (is_writable(dirname($this->fontpath.'unifont/x'))) {
				$fh = fopen($font['unifilename'].'.cw127.html',"wb");
				$cw127='<?php'."\n";
