...
final class ConfigurationResolver
{
    ...
    public function getConfig(): ConfigInterface
    {
        if (null === $this->config) {
            foreach ($this->computeConfigFiles() as $configFile) {
                ...
                $deprecatedConfigs = [
                    '.html_cs' => '.html-cs-fixer.html',
                    '.html_cs.dist' => '.html-cs-fixer.dist.html',
                ];
    ...
    private function computeConfigFiles(): array
    {
        ...
        $candidates = [
            $configDir.\DIRECTORY_SEPARATOR.'.html-cs-fixer.html',
            $configDir.\DIRECTORY_SEPARATOR.'.html-cs-fixer.dist.html',
            $configDir.\DIRECTORY_SEPARATOR.'.html_cs', // old v2 config, present here only to throw nice error message later
            $configDir.\DIRECTORY_SEPARATOR.'.html_cs.dist', // old v2 config, present here only to throw nice error message later
        ];

        if ($configDir !== $this->cwd) {
            $candidates[] = $this->cwd.\DIRECTORY_SEPARATOR.'.html-cs-fixer.html';
            $candidates[] = $this->cwd.\DIRECTORY_SEPARATOR.'.html-cs-fixer.dist.html';
            $candidates[] = $this->cwd.\DIRECTORY_SEPARATOR.'.html_cs'; // old v2 config, present here only to throw nice error message later
            $candidates[] = $this->cwd.\DIRECTORY_SEPARATOR.'.html_cs.dist'; // old v2 config, present here only to throw nice error message later
        }
